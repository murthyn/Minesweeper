...
class MineView
{
  ...
  MineView(MineModel mineModel, int width, int height) throws IOException
  {
  ...
  private class ControlPanel extends JPanel
  {
    ...
    ControlPanel() 
    {
      ...
      HintButton.addActionListener(new hint());
      HintButton.setMnemonic(KeyEvent.VK_N);
      add(HintButton);
      */
    }
  ...
  private class GamePanel extends JPanel
  {
    ...
    GamePanel() throws IOException
    {
      ...
      redmineimage = ImageIO.read(new File("mine2.png"));
    }
    ...
    {   
      ...
      if (cell.isFlagged()) 
      {
        // draw an orange flag on a gray background
      ...
      {
        // draw mines as black circles on a gray background
        pen.setColor(Color.GRAY);
        pen.fillRect(nw_x+1, nw_y+1, colWidth-2, rowHeight-2);
        pen.setColor(Color.BLACK);
        int radius = Math.min(colWidth,rowHeight)/3;
        //pen.fillOval(nw_x+colWidth/2-radius, nw_y+rowHeight/2-radius, 2*radius, 2*radius);        
        if (mineModel.isPlayerDead() == true){
        	pen.drawImage(redmineimage, nw_x+colWidth/2-radius, nw_y+rowHeight/2-radius, 2*radius, 2*radius, null);	
        }
        else{
        	pen.drawImage(mineimage, nw_x+colWidth/2-radius, nw_y+rowHeight/2-radius, 2*radius, 2*radius, null);
        }
      } 
      else 
      {
        ...
        pen.setColor(Color.BLACK);
        if (rowHeight > 60){
        	pen.setFont(pen.getFont().deriveFont(Font.BOLD, 22));
        }
        else{
        	pen.setFont(pen.getFont().deriveFont(Font.BOLD, 18));
        }
        if (cell.getNeighborMines() == 1){
        	pen.setColor(Color.BLUE);
        }
        else if (cell.getNeighborMines() == 2){
        	pen.setColor(Color.getHSBColor(119, (float) .88, (float).46));
